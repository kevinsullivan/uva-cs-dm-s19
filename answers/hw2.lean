...
Answer here:  G = ({1, 2, 3}, { (1, 10), (2, 20), (3, 30) }, ℕ)

Yes, it's defined for every value in its domain of definition.
...
2. Give an example of a value in the domain of definition of the function, F, 
as defined in the reading, that proves that F is strictly partial, i.e., 
for which F is not defined.

Answer here:  F = (ℕ, { (1, 10), (2, 20), (3, 30) }, ℕ ). F is partial. It's
not defined for 0 or any other value that's not 1, 2, 3.
...
Answer here: Any natural number other than 10, 20, 30.
...
Answer here: No, we already said it's total.
...
value in its domain of definition.

Answer here: Any number that's not 10, 20, 30
...
Answer here: A function, f, is injective if the whenever two values map
to the same image they are the same value. We will accept any reasonable
...
Answer here: Yes it's injective (it passes the horizontal line test.) We
will take any reasonable explanation. Yes it's surjective, in that it covers
the whole co-domain.
...
Answer here: No, it's an ellipse. Doesn't pass vertical line test.
...
-- Rubric: We will accept your answer as long as Lean does. We give
-- examples of answers in this key. Other anwers are possible.
